...
[trainingSet, ValandtestSet] = splitEachLabel(imds, 0.4, 'randomize');
[testSet, validationSet] = splitEachLabel(ValandtestSet,0.5,'randomize');
...
%% 4.1 KNN Classifier
% Train a KNN Classifier with 10 neighbors and cosine distance as the
% distance metric. There is a need to transpose trainingFeatures matrix 
% to match the arrangement used for training features in a KNN Classifier.
classifier_knn = fitcknn(trainingFeatures, trainingLabels, ...
    'NumNeighbors',10,'Distance','cosine');

%% 4.2 Naive Bayes Classifier
% Train a Naive Bayes Classifier to perform the task of classification 
% using conditional probabilites. There is a need to transpose 
% trainingFeatures matrix to match the arrangement used for training 
% features in a Naive Bayes Classifier.
classifier_nbc = fitcnb(trainingFeatures, trainingLabels);
...
% train images
predictedLabels_train_knn = predict(classifier_knn, trainingFeatures);
predictedLabels_train_nbc = predict(classifier_nbc, trainingFeatures);

% test images
predictedLabels_test_knn = predict(classifier_knn, testFeatures);
predictedLabels_test_nbc = predict(classifier_nbc, testFeatures);
...
confMat_train_knn = confusionmat(trainingLabels, predictedLabels_train_knn);
confMat_train_nbc = confusionmat(trainingLabels, predictedLabels_train_nbc);

% Accuracy
accuracy_train_knn = (confMat_train_knn(1,1) + confMat_train_knn(2,2)) / sum(sum(confMat_train_knn))
accuracy_train_nbc = (confMat_train_nbc(1,1) + confMat_train_nbc(2,2)) / sum(sum(confMat_train_nbc))

% Convert confusion matrix into percentage form
bsxfun(@rdivide,confMat_train_knn,sum(confMat_train_knn,2))
bsxfun(@rdivide,confMat_train_nbc,sum(confMat_train_nbc,2))
% Test Data

% Tabulate the results using a confusion matrix.
confMat_test_knn = confusionmat(testLabels, predictedLabels_test_knn);
confMat_test_nbc = confusionmat(testLabels, predictedLabels_test_nbc);

% Accuracy
accuracy_test_knn = (confMat_test_knn(1,1) + confMat_test_knn(2,2)) / sum(sum(confMat_test_knn))
accuracy_test_nbc = (confMat_test_nbc(1,1) + confMat_test_nbc(2,2)) / sum(sum(confMat_test_nbc))

% Convert confusion matrix into percentage form
bsxfun(@rdivide,confMat_test_knn,sum(confMat_test_knn,2))
bsxfun(@rdivide,confMat_test_nbc,sum(confMat_test_nbc,2))

% CROSS VALIDATION ERROR

% Cross validate the KNN classifier using the default 10-fold cross 
% validation. Examine the classification error.
cv_knn = crossval(classifier_knn);
classError = kfoldLoss(cv_knn)

% Cross validate the Naive Bayes classifier using the default 10-fold cross 
% validation. Examine the classification error.
cv_nbc = crossval(classifier_nbc);
classError = kfoldLoss(cv_nbc)

%% 5.3: Test the classifiers on an unseen image
newImage = './doge.jpg'; % any cat or dog image should do!
img = readAndPreprocessImage(newImage);
imageFeatures = activations(convnet, img, featureLayer);
label_knn = predict(classifier_knn, imageFeatures);
label_nbc = predict(classifier_nbc, imageFeatures);

% Display test image and assigned label
figure, imshow(img), title(strcat('Predicted by KNN Classifier: ',char(label_knn))); 
figure, imshow(img), title(strcat('Prediction by Naive Bayes Classifier: ',char(label_knn)));
